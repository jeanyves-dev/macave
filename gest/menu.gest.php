...
else
{
	if (isset($_POST['demenu']) AND $_POST['demenu'] <>"")
	{
		...
		{
			$donnees  = array('remenu'=>$_POST['remenu'],'demenu'=>$_POST['demenu'],'limenu'=>$_POST['limenu'],'norang'=>$_POST['norang']);
			$menu     = new menu($donnees);
			$menu_dao->update($menu);
		}
		else
		{
			$donnees  = array('demenu'=>$_POST['demenu'],'limenu'=>$_POST['limenu'],'norang'=>$_POST['norang']);
			$menu     = new menu($donnees);
			$menu_dao->add($menu);
		}
